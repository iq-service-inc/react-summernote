        ...
        context.memo('button.captionIt', function () {
          var button=ui.button({
            ...
            click: self.wrapCommand(function () {
              ...
              const isFigure = dom.makePredByNodeName('FIGURE')
              var $figure = $(dom.ancestor($img[0], isFigure))
              var $anchor = $(dom.ancestor($img[0], dom.isAnchor))
              if ($figure.length) {
                $figure.find('figcaption').remove();
                if ($anchor.length) {
                  $anchor.unwrap('figure')
                }
                else {
                  $img.unwrap('figure')
                }
                ...
                if ($anchor.length) {
                  $anchor.wrap('<figure class="' + options.captionIt.figureClass + '"></figure>')
                  let $newFigure = $anchor.parent();
                  let $caption = '<figcaption class="' + options.captionIt.figcaptionClass + '">' + captionText + '</figcaption>'
                  if (options.captionIt.captionInAnchor) {
                  ...
                  let $newFigure = $($img).wrap('<figure class="' + options.captionIt.figureClass + '"></figure>').parent();
