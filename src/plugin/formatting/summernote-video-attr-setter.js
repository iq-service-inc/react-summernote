...
(function (factory) {
  ...
  $.extend($.summernote.plugins, {
    'videoAttrSetter': function (context) {
      ...
      this.initialize = function () {
        ...
        if (options.videoAttrSetter.showAutoplay === true) {
          body += '<div class="form-group clearfix">' +
            ...
            '      <label>' +
            '        <input type="checkbox" id="note-video-attributes-autoplay-checkbox" class="note-video-attributes-autoplay-checkbox">' + ` ${lang.videoAttrSetter.autoplay}` + '</label>' +
            '    </div>' +
            '  </div>' +
            '</div>';
        }
        ...
        if (options.videoAttrSetter.showLoop === true) {
          body += '<div class="form-group clearfix">' +
            ...
            '    <div class="checkbox checkbox-success">' +
            `      <label ${options.id}>` +
            '        <input type="checkbox" id="note-video-attributes-loop-checkbox" class="note-video-attributes-loop-checkbox">' + ` ${lang.videoAttrSetter.loop}` + '</label>' +
            '    </div>' +
            '  </div>' +
            '</div>';
        }
        ...
        if (options.videoAttrSetter.showMute === true) {
          body += '<div class="form-group clearfix">' +
            ...
            '    <div class="checkbox checkbox-success">' +
            '      <label>' +
            '        <input type="checkbox" id="note-video-attributes-mute-checkbox" class="note-video-attributes-mute-checkbox">' + ` ${lang.videoAttrSetter.muted}` + '</label>' +
            '    </div>' +
            '  </div>' +
            '</div>';
        }
        ...
        if (options.videoAttrSetter.showPreload === true) {
          body += '<div class="form-group">' +
            ...
            '  <div class="input-group col-xs-9">' +
            '    <select id="note-video-attributes-preload" class="note-video-attributes-preload form-control col-xs-6">' +
        ...
        this.editBtn = $('<button>')
          .addClass('note-btn btn btn-light btn-sm faq-video-attr-setter-popover-edit-btn')
          .attr('data-toggle', 'tooltip')
          .attr('title', lang.videoAttrSetter.edit)
          .html(`<i class="note-icon">${options.videoAttrSetter.editIcon}</i>`)

        this.deleteBtn = $('<button>')
          .addClass('note-btn btn btn-light btn-sm faq-video-attr-setter-popover-delete-btn')
          .attr('data-toggle', 'tooltip')
          .attr('title', lang.videoAttrSetter.delete)
          .html(`<i class="note-icon">${options.videoAttrSetter.deleteIcon}</i>`)

        const btnContainer = $('<div>').addClass('btn-group note-btn-group').append(this.editBtn).append(this.deleteBtn);

        this.faqRefPopoverDiv = $('<div>').addClass('popover').css({ display: 'none', padding: '5px' });
        this.faqRefPopoverDiv.append(btnContainer)
        this.faqRefPopoverDiv.appendTo($editor);

        this.editBtn.tooltip();
      ...
      this.events = {
        'summernote.disable summernote.dialog.shown summernote.blur summernote.keyup summernote.scroll summernote.change': function () {
          self.faqRefPopoverDiv.css({ display: 'none' });
        },
        'summernote.mousedown': function (we, e) {
          element = e.target;
          sourceElement = element.querySelector('source');
          var srcValue = !sourceElement ? null : sourceElement.getAttribute('src');
          var editorOffset = $editor.offset();

          var relativeX = e.pageX - editorOffset.left;
          var relativeY = e.pageY - editorOffset.top;

          if (element.nodeName.toLowerCase() === 'video') {
            e.preventDefault();
            self.faqRefPopoverDiv.css({ display: 'block', padding: '5px', top: `${relativeY}px`, left: `${relativeX}px` })
